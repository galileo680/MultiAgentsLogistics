...
def run_simulation(num_steps=100):
    ...
    for step in range(num_steps):
        ...
        agent_positions = [
            (agent.position[0], agent.position[1], agent.label) for agent in all_agents
        ]
        grid.render_console(agent_positions)
        for t in transporters:
            print(
                f"{t.agent_id} Busy: {t.is_busy}, Reward: {t.total_reward}, Reached Goal Count: {t.reached_goal_count}"
            )
        ...
        metrics_collector.log_step(step, transporters)
    ...
    for t in transporters:
        print(
            f"{t.agent_id}: Całkowita nagroda = {t.total_reward}, Epsilon = {t.epsilon:.4f}"
        )
        t.brain.save_table(f"{t.agent_id}_brain.npy")

    metrics_collector.save_data()
